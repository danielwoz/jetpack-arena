...
function makePlayer(id: number, x: number, y: number, weapon: WeaponId, aim: number): CombatPlayer {
  const state: PlayerState = {
    ...
    slots: ['m4a1', 'shotgun', weapon] as Loadout, slotIdx: 2,
...
function scenario(
  ...
  const shooter = makePlayer(1, 500, 500, 'm4a1', 0);
  ...
  const bullets: Bullet[] = [{
    ...
    weapon: 'm4a1', owner: 1, rewindOff: 0, life: 1, dist: 0, hit: [], spent: false,
  }];
...
// melee: a pan swing at arm's length is lethal; out of range whiffs
{
  ...
  const target = makePlayer(2, 2300, 1950, 'm4a1', Math.PI);
  const far = makePlayer(3, 2450, 1950, 'm4a1', Math.PI); // 195 away — safe
  const events: GameEvent[] = [];
  const lagcomp = new LagComp();
  const cw: CombatWorld = {
    tick: 100, lagcomp, events, world: new World(),
...
{
  const shooter = makePlayer(1, 500, 500, 'm4a1', 0);
  const victim = makePlayer(2, 2300, 1950, 'm4a1', Math.PI);
  const events: GameEvent[] = [];
  const lagcomp = new LagComp();
  const cw: CombatWorld = {
    tick: 100, lagcomp, events, world: new World(),
  ...
  const b1: Bullet[] = [{ x: 2100, y: 1930, vx: 3400, vy: 0, weapon: 'm4a1', owner: 1, rewindOff: 0, life: 1, dist: 0, hit: [], spent: false }];
  ...
  victim.state.hp = 100;
  events.length = 0;
  const b3: Bullet[] = [{ x: 2100, y: 1970, vx: 3400, vy: 0, weapon: 'm4a1', owner: 1, rewindOff: 0, life: 1, dist: 0, hit: [], spent: false }];
...
{
  const shooter = makePlayer(1, 500, 500, 'm4a1', 0);
  const v1 = makePlayer(2, 2300, 1950, 'm4a1', Math.PI);
  const v2 = makePlayer(3, 2400, 1950, 'm4a1', Math.PI);
  const events: GameEvent[] = [];
  const lagcomp = new LagComp();
  const cw: CombatWorld = {
    tick: 100, lagcomp, events, world: new World(),
  ...
  const b: Bullet[] = [{ x: 2100, y: 1950, vx: 3400, vy: 0, weapon: 'm4a1', owner: 1, rewindOff: 0, life: 1, dist: 0, hit: [], spent: false }];
